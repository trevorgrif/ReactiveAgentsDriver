...
function _create_town!(town_type::String, connection::DuckDB.DB)
    ...
    elseif town_type == "large"
        ...
        @error "Invalid town type $(town_type). Must be 'small' or 'large'"
    end

    # Construct town and assign model meta-data
    model = populate(town_type)
    ...
    DuckDB.append(appender, length(model.schools))
    ...
    DuckDB.append(appender, join(serialize(model), ","))
...
function _create_network!(town_id::Int, duration::Int, connection::DuckDB.DB; STORE_NETWORK_SCM = false)
    ...
    simulate!(model, duration=duration)
...
function _create_behaved_network!(network_id::Int, mask_distribution_type::String, vax_distribution_type::String, mask_portion::Int, vax_portion::Int, connection::DuckDB.DB)
    ...
    mask!(model, mask_portion, mask_distribution_type)
    vaccinate!(model, vax_portion, vax_distribution_type)
...
function _create_epidemic!(model, connection::DuckDB.DB; STORE_EPIDEMIC_SCM=false)
    ...
    # Append Epidemic level data
    _append_epidemic_level_data(model, STORE_EPIDEMIC_SCM, connection)

    return model.epidemic_id
end

function _create_epidemic_distributed!(model, epidemic_runs::Int; STORE_EPIDEMIC_SCM=false)
...
function _append_epidemic_level_data(model, STORE_EPIDEMIC_SCM::Bool, db::DuckDB.DB)
    # Append EpidemicDim data
    ...
    # Append EpidemicLoad data
    appender = DuckDB.Appender(db, "EpidemicLoad")
    for row in eachrow(model.epidemic_data)
        DuckDB.append(appender, model.epidemic_id)
        DuckDB.append(appender, row[1])
        DuckDB.append(appender, row[2])
    ...
    # Append TransmissionLoad data
    appender = DuckDB.Appender(db, "TransmissionLoad")
    for row in eachrow(model.TransmissionNetwork)
        DuckDB.append(appender, model.epidemic_id)
        DuckDB.append(appender, row[1])
        DuckDB.append(appender, row[2])
...
#     model_raw , townDataSummaryDF, businessStructureDF, houseStructureDF = populate(joinpath("lib", "RuralABM", "data", "example_towns", "$(town)_town", "population.csv"), joinpath("lib", "RuralABM", "data", "example_towns", "$(town)_town", "businesses.csv"))
